// write hello world program
// print out a greeting with a name e.g Hello John Doe
// how to print to console in javascript

//Datatypes

// String example, "Movefdfgdgfdfgdffgdf"  or 'somestring'
var name = "Femi";

// Numbers e.g float or integer e.g 12.02, 12
var x = 2;

// Arrays e.g list of product names ["product1", "product2"],
var numbers = [2, 4, 5, 6];
var names = ["name1", "name2"];

// Object e.g {"name": "some name","department": "some", "type", id: 12345}
var girl = { name: "Aliona", "sex": "Female" };
var car = { make: "Audi", chasisNumber: 12121212, color: "Red" };
var student = { id: 343434, year: 1, course: "Computer Science" };

// Function
var identificationNumber = function (person) {
  return person.id;
}
identificationNumber({ id: 1212 });
// A variablle as address to some data in memory 


//Arithmetc operators
// +, -, %, /, >=, <=, ==, ===

//Logical operators
// and && -> all candidates must be true
// or || -> at least one candidate must be true
// not ! -> e.g
//  -> if (!check_login()) do: xcdgsdfdgdf


// why do we need git (github)
// Git versions 
// => (it is keeping track of our revisions)
// => (it is keeping track of our revisions)
// =>


//Git | SVN
// git init # cretes new repository
// git remote add origin repo-path # Binds our local
// git status
// git add < filename > # add changes to be ttracked
// git commit -m "<some message>" # add new changes to the repo
// git diff # shows you the difference the in change and the last tracked point
// git push #transfers the local changes to the remote repo
// git clone <repository url> # Clones a repository to current working direcory on local
// git pull # will fetch the remote repository to the local



//Begin
//accept a name
// prepare greeting
// print out greeting
// end
var name = "John Doe";
var greeting = "Hello " + name;
console.log(greeting);
console.log("End of Class");


